...
from scripts.run_jeju_context_experiment import (
    CTX,
    DATA,
    H3,
    OUT,
...
def main():
    ...
    assert len(p) == 8 * 715 and len(h) == 8 * 6953
    ...
    oldh = (
        pl.read_parquet(V4 / "horse_predictions.parquet")
    ...
    assert (
        ...
        and len(ledger["estimator_fits"]) == 16
        and len(ledger["calibration_fits"]) == 16
    )
    assert all(
        e["status"] == "complete" for e in ledger["estimator_fits"] + ledger["calibration_fits"]
    )
    assert len(replay) == 16 and ledger["protocol_sha256"] == sha(OUT / "protocol.json")
