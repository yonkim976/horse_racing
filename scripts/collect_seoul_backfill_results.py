"""Archive annual official Seoul race-result responses in a separate research path."""
...
DEFAULT_ROOT = Path("data/research/seoul_backfill_20260915_v1")
...
def collect(start: int, end: int, root: Path) -> None:
    ...
    with KraApiClient(
        ...
        for year in range(start, end + 1):
            ...
            page_no = 1
            while True:
                path = root / "raw_results" / str(year) / f"page_{page_no:04d}.json"
                if path.exists():
                ...
                if any(
                    row.get("meet") != "서울" or int(row["rcDate"]) // 10000 != year
                    for row in batch
                ):
                ...
                event = {
                    ...
                    "scope": {"meet": 1, "rc_year": year},
                    "page": page_no,
                    "requested_page_size": 20000,
                    "actual_page_size": actual_size,
                    "total_count": count,
                    "response_rows": len(batch),
                    "collected_at_utc": datetime.now(UTC).isoformat(),
                    "source_retrieved_at_ms": fetched_at,
