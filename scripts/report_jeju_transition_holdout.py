...
def main():
    ...
    for candidate, reference in pairs:
        ...
        row = {
            "candidate": candidate,
            "reference": reference,
            "interpretation": "Fixed2026 comparison;95% diagnostic intervals; "
            "two primary set contrasts use97.5% below",
        }
        for j, k in enumerate(keys):
            row[k] = {
                ...
                "primary_day_ci_97_5": np.quantile(
                    day_boot[i, :, j] - day_boot[ref, :, j], [0.0125, 0.9875]
                ).tolist()
                if k == "set_hit"
                and candidate in ["HY_TRANSITION", "HY_TRANSITION_CONTEXT"]
                and reference == "HY_R_FORM"
                else None,
            }
                ...
                reliability.append(
                    {
                        ...
                        if source == "PL" and m.startswith(("J_", "HY_", "C_"))
                        else source,
                        "bucket": bucket,
                        "races": len(part),
    ...
    save(
        "analysis_metadata.json",
        {
            ...
            "probability_population": (
                ...
                "Brier/logloss/reliability; hitandthresholdcountsall510"
            ),
        },
    )
    print(
