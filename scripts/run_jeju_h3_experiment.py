"""Declared H3 conditions and relative-state ablation on three targets."""
...
def main():
    ...
    assert sha(H3 / "manifest.json") == protocol["h3_feature_manifest_sha256"]
    ...
    for path in [
        ...
        ROOT / "pyproject.toml",
        ROOT / "src/horse_racing/analysis/jeju_h3_features.py",
        ROOT / "scripts/build_jeju_h3_features.py",
    ...
    frame = states.join(
        labels.select("entry_id", "label_top3", "finish_position"), on="entry_id", validate="1:1"
    ).sort("race_id", "horse_id")
    frame = frame.join(pl.read_parquet(H3 / "features.parquet"), on="entry_id", validate="1:1")
    race_meta = {
        r["race_id"]: r
    ...
    old_scores = pl.read_parquet(OLD / "horse_predictions.parquet").filter(
    ...
    for (model, fold), s in old_scores.partition_by(["model", "fold"], as_dict=True).items():
        ...
        evaluate(
            ...
            s["score"].to_numpy(),
            float(s["beta"][0]),
            s["native_place_probability"].to_numpy() if model.startswith("P_") else None,
        )
    for fold in sorted(folds["fold_id"].unique().to_list()):
        ...
        for arm, family in [("H3", "P"), ("H3", "R"), ("H3R", "P"), ("H3R", "R")]:
            name = family + "_" + arm
            features = [
                r["feature_name"]
                for r in registry
                if r["feature_name"] not in protocol["aliases_removed"]
            ]
            features += protocol["arms"][arm]
            pre = FitPreprocessor().fit(parts["fit"], features)
            xs = {k: pre.transform(v) for k, v in parts.items()}
            models = []
            for seed in [17, 43]:
                cls = lgb.LGBMClassifier if family == "P" else lgb.LGBMRanker
                extra = {} if family == "P" else {"label_gain": [0, 1, 3, 7]}
                m = cls(
                    objective="binary" if family == "P" else "lambdarank",
                    random_state=seed,
                    verbosity=-1,
                    **protocol["params"],
                    **extra,
                ...
                if family == "P":
                    ...
                    sizes = {
                        k: [hi - lo for lo, hi, _ in race_groups(v, truth)]
                        for k, v in parts.items()
                    }
                    m.fit(
                        xs["fit"],
            ...
            native = nativecal.predict(raw["evaluation"]) if nativecal is not None else None
    ...
    assert len(expected) == 715 and p["model"].n_unique() == 6
    for model in p["model"].unique():
        part = p.filter(pl.col("model") == model)
        assert len(part) == 715 and set(part["race_id"]) == expected
    assert len(ledger["estimator_fits"]) == 32 and len(ledger["calibration_fits"]) == 24
    ledger.update(
        status="complete",
