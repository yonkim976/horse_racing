"""Context/form ablations and preserved-set order hybrids, 2025 development only."""
...
OUT = ROOT / "data/research/jeju_native_context_experiment_v5_20260916"
...
def main():
    ...
    for path in [
        Path(__file__),
        ROOT / "scripts/report_jeju_context_experiment.py",
        ROOT / "scripts/verify_jeju_context_experiment.py",
    ...
    oldp = pl.read_parquet(V4 / "race_predictions.parquet").filter(
        pl.col("model").is_in(protocol["references"])
    )
    oldh = pl.read_parquet(V4 / "horse_predictions.parquet").filter(
        pl.col("model").is_in(protocol["references"])
    )
    ...
    base_features = protocol["base_features"]
    for fold in sorted(folds["fold_id"].unique()):
        ...
        for name, extras in [
            ("R_CTX", protocol["context_features"]),
            ("R_FORM", protocol["context_features"] + protocol["form_features"]),
        ]:
        ...
        for name, parent in [("HY_R_H3", "R_H3"), ("HY_R_FORM", "R_FORM")]:
    ...
    assert len(p) == 8 * 715 and p["model"].n_unique() == 8
    assert all(
        p.filter(pl.col("model") == m)["race_id"].n_unique() == 715 for m in p["model"].unique()
    )
    assert len(ledger["estimator_fits"]) == 16 and len(ledger["calibration_fits"]) == 16
    ledger.update(
        status="complete",
