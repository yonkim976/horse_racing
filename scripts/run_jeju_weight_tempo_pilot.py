"""Fixed measured-weight and official-tempo pilot; no live deployment."""
import hashlib
import json
import pickle
...
FEAT=ROOT/'data/research/jeju_weight_tempo_features_20260917'
OUT=ROOT/'data/research/jeju_weight_tempo_pilot_20260917'
...
def main():
    ...
    candidates={'WEIGHT':WEIGHT,'TEMPO':TEMPO}
    save('protocol.json',dict(created_at=datetime.now(UTC).isoformat(),evaluation_status='previously_seen_retrospective_development',
        ...
        plan_sha256=sha(ROOT/'docs/JEJU_WEIGHT_TEMPO_READINESS_PLAN_2026-09-17.md'),
        candidates=candidates,seeds=[17,43],rank_params=config['rank_params'],order_params=config['order_params'],
        main_contrasts='A2+B2+C1+BrierW1, day bootstrap5000 Bonferroni6; measured historical weight, no automatic deployment'))
    ...
    features=obase['features']+WEIGHT
    pre=FitPreprocessor().fit(parts['fit'],features)
    xs={k:pre.transform(v) for k,v in parts.items()}
    ...
    for seed in [17,43]:
        ...
        models.append(model)
        ledger.append(dict(model='ORDER_WEIGHT',seed=seed,diagnostics={k:v for k,v in vars(model).items()
                      if k.endswith('_') and isinstance(v,(str,int,float,bool,type(None)))}))
        save('fit_ledger.json',ledger);print('ORDER_WEIGHT',seed,'done',flush=True)
    ...
    (OUT/'bundles/ORDER_WEIGHT.pkl').write_bytes(pickle.dumps(orders['NEW']))
    allscores=[]
    ...
    primary=[(name+'__BASE',metric) for name in ['WEIGHT','TEMPO'] for metric in ['pick_hit','set_hit']]+[('BASE__NEW','order_hit'),('WEIGHT__BASE','place_brier')]
    comparisons=[]
    for name,metric in primary:
        ...
        if metric=='place_brier': delta=-delta  # Positive always denotes improvement.
